...
def evaluate(Y, m_dots, D_dots, m_t, D_t):
    ...
    print('\nДоверительный интервал для оценки МО={:.4f} СВ при известной D={:.6f} для различных уровней '\
    ...
    print('\nДоверительный интервал для оценки D={:.4f} СВ при известном M={:.6f} для различных уровней '\
...
def evaluate_R(n, R_dots, R_t):
    '''
        Интервальные оценки корреляции СВ Х и У

        n - кол-во СЧ Х (Y) 
        R_dots - точечная оценка корреляции СВ Х и Y
        R_t - теоретическое значение корреляции СВ Х и Y
    '''
    level, z1, z2= R_intervals(n, R_dots)

    print('\nДоверительный интервал для оценки r={:.4f} СВ X и Y для различных уровней значимости:'.format(R_t))
    table_for_confidence_interval_R(level, z1, z2, R_t)

    return [level, z1, z2]


def table_for_confidence_interval_R(level, z1, z2, R_t):
    '''Таблица зависимости доверительного интервала корреляции от уровня значимости'''
    print("Уровень значимости   Доверительный интервал МО   Накрывает MO?")
    for i in range(len(level)):
        print("%10.2f" % level[i], 
              "           [{:.4f}, {:.4f}]".format(z1[i], z2[i]),
              "          ", ('Да' if z1[i] <= R_t <= z2[i] else 'Нет'))

def R_intervals(n, r):
    '''
            Поиск осн. параметров для интервальной оценки корреляции
        n - кол-во СЧ Y
        r  - см. ф. evaluate()

        return level, z1, z2  - см. ф. table_for_confidence_interval_R()
    '''
    # значения альфа - уровень значимости
    level = [0.1, 0.05, 0.01]
    t_Laplas = [1.65, 1.96, 2.58]

    z = np.log((1+r) / (1-r)) / 2
    
    z1, z2 = [], []
    for i in range(len(level)):
        z1.append(z - r/(2*(n-1)) - t_Laplas[i]/np.sqrt(n-3))
        z2.append(z - r/(2*(n-1)) + t_Laplas[i]/np.sqrt(n-3))
